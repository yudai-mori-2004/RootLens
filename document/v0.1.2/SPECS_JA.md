...
ブロックチェーンを使う理由は、RootLens 自身にもライセンス記録の隠蔽・捏造ができない構造を作るためである。License NFT の発行数、保有者、収益の流れはすべてオンチェーンで公開されており、第三者が独立に検証できる。AI 企業は監査時に「このデータを使う権利がある」ことを License NFT で証明できる。収益分配は 95:5 の比率でスマートコントラクトにより自動執行され、RootLens が分配比率を操作することはできない。
...
1. **撮影証明（コンテンツ ID）**：C2PA 署名が検証済みであり、正規のデバイスで撮影された改ざんのないコンテンツであることの証明。TitleCore コレクションに属することで、RootLens の TEE ノードから発行されたことが保証される。コンテンツの一意な識別子として機能する。
2. **ライセンス発行権の鍵**：この Root NFT の delegate は、対応するコンテンツに対して License NFT の発行に co-sign する権限を持つ（§5 参照）。この権限は、§4.4 の著作権利用許諾に基づく。
...
#### 4.4.2 ToS のバージョン管理と公開

ToS は version 単位で管理し、各 version は immutable な R2 URL に置かれ、`keccak256` ハッシュが識別子となる。
...
ユーザー × ToS version 単位で 1 回の同意を記録する。Web2 SaaS (Google / Twitter / Apple 等) と同等の click-through 方式。

1. ユーザーがアプリを初回起動した時、又は ToS が新 version に更新された後の初回起動時、アプリが現行 ToS の (`version`, `tos_hash`, `tos_url`, `tos_text` 全文) を表示し、明示的同意を求める（チェックボックス + 全文スクロール完了）。
2. RootLens は同意イベントを server-side に記録する:
...
4. ToS が新 version に更新されたら、次回ユーザーがアプリを起動した際にステップ 1 から再実行。

**証拠保全**は web2 SaaS と同等の事業者責任で行う。`tos_hash` を記録することで「どの version に同意したか」が後から検証可能となる。RootLens は append-only に近いストレージ (immutable backups + audit log) で記録を保管する義務を負う。

> **設計判断**: 当初 ed25519 wallet 署名や Solana memo program を経由した on-chain 化を検討したが、以下の理由で web2 標準を採用した。
> - ToS 同意は「ユーザー × version」で年に数回程度しか発生しないイベント。1 同意ごとに wallet 署名を要求するのは UX 過剰
> - License NFT 発行 (§5.3) は B2B 取引であり、当事者は KYC 済 AI 企業。Licensor 側が licensee 側の同意の有無を争うシナリオは存在しない
> - RootLens 側で個人ユーザーを著作権侵害で訴える主要シナリオは想定されない (撮影者は寄り添うべき相手であり、係争相手ではない)
> - Web2 SaaS の同意記録は十数年の judicial scrutiny を経て十分な証拠能力が認められている
...
> **§X 著作権利用許諾**
> 
> 1. 撮影者は、本サービスを通じてアップロードし、Root NFT として発行された各動画コンテンツ（以下「対象コンテンツ」）について、当該 Root NFT の delegate（Bubblegum 標準機能により Root NFT に対し delegate として設定されている者）に対し、以下の権限を付与する。
> 
>    a. 対象コンテンツの著作権の利用を、第三者に対してサブライセンスする権利。サブライセンスは非独占的、全世界的とする。
> 
>    b. サブライセンスは License NFT の発行を通じてのみ行う。すなわち delegate は、本サービスの License NFT スマートコントラクト (`license_nft` プログラム) を介して License NFT を発行することによってのみ、第三者にサブライセンスを付与する。それ以外の方法でのサブライセンス付与は本許諾の範囲外であり、本許諾を根拠とすることはできない。
> 
>    c. サブライセンスの内容は、License NFT の発行時点でその NFT のメタデータ URI が指す license terms（§5.5.3）が定めるところによる。delegate は License NFT 発行時に複数のライセンス種別から選択する権限を持つ。
> 
> 2. 第 1 項に基づく許諾は、サブライセンスの目的にのみ使用される。delegate は、付与された権限を License NFT 発行以外の目的（Root NFT の transfer、burn、freeze、その他の処分、又は対象コンテンツの直接的な複製・配布等）に使用してはならない。これに違反する行為は、本許諾の範囲外であり、別途撮影者の著作権侵害となる。
> 
> 3. delegate の地位は、Bubblegum delegate 命令により撮影者がいつでも変更又は解除（アンステーク）できる。
> 
>    a. delegate 変更・解除以後、元 delegate は対象コンテンツに対する **新規** License NFT の発行に co-sign する権限を失う。
> 
>    b. delegate 変更・解除前に発行済みの License NFT は影響を受けない。当該 License NFT に紐づく license terms に定められた範囲・期間において、ライセンスは存続する。
> 
> 4. 撮影者は、本許諾を License NFT が一切発行されていない時点においていつでも撤回できる（delegate のアンステークが撤回方法となる）。発行済みの License NFT は撤回の対象外であり、それぞれの license terms に従う。
> 
> 5. 本許諾は、撮影者が本 ToS の特定 version に同意した時点で、当該 version 同意の対象コンテンツについて成立する。
...
ある License NFT の正当性は、以下のチェーンを辿ることで第三者が検証できる。

1. License NFT の URI から `?root_mint=<root_asset_id>` をパース → 紐づく Root NFT を特定（§5.5.3 Layer 1）
2. Root NFT を DAS から取得 → owner（撮影者の wallet）と TitleCore 所属を確認
3. RootLens の公開 ToS hash チェーンログから、撮影者 wallet の同意レコード（version、hash、署名）を取得し、同意の事実と version を確認
4. 当該 ToS version の条文（§4.4.4）に基づき、当時の delegate（License NFT 発行 tx の delegate signer）が正当なサブライセンス権を有していたことを確認
5. License NFT の URI が指す license JSON で、最終ライセンシー（License NFT 保有者）への許諾範囲を確認

ステップ 1〜5 すべてが整合する場合、License NFT 保有者の権限は、ToS 同意 → サブライセンス → License 発行 → 保有 という連鎖で法的に正当化される。
...
購入者の署名と delegate の署名の両方を要求する。引数として、Root NFT の Bubblegum proof（root, nonce, index, data_hash, creator_hash, asset_data_hash, flags）、TitleCore コレクションの公開鍵、License NFT のメタデータ（uri, name）、価格を受け取る。
...
- Root NFT が TitleCore コレクションに属すること（引数の root_collection が Config の title_core_collection と一致、かつ leaf の collection_hash と整合）。これにより、Root NFT が RootLens の TEE ノードから正規に発行されたものであることが保証される。
...
1. **License NFT 発行**: 1 個の License NFT（cNFT）を、license_tree で指定された Merkle Tree に購入者宛てでミントする。Bubblegum V2 の MintV2 を CPI 経由で呼ぶ。
   ...
   - **`asset_data` フィールドに `root_asset_id` の 32 バイトを書き込む**。Bubblegum がこれを keccak256 ハッシュ化して `asset_data_hash` として leaf に焼き込む。これにより License NFT が特定の Root NFT に対するライセンスであることが暗号学的に固定される（§5.5.3 参照）。
...
4. price の 95% を、ステーカーのユーザー収益アカウントの残高に加算する。
5. price の 5% を、delegate のウォレットに即時送金する。
...
delegate の 5% はプール内に蓄積せず、issue_license 実行時に delegate のウォレットに即時送金する。

### 5.4.2 分配比率

初期の分配比率は ステーカー 95% : delegate 5% とする。この比率はコントラクトのパラメータとしてオンチェーンに公開され、第三者が検証可能である。
...
> **設計補足**: 理想的には Bubblegum V2 の `asset_data` フィールドに root_asset_id の raw bytes を渡し、Bubblegum 側で `keccak256(asset_data)` を `leaf.asset_data_hash` に焼き込ませるのがより構造的にクリーンである（`MetadataArgsV2.uri` の semantic 用途と分離できる）。しかし 2026-05 時点で devnet 上の deployed mpl-bubblegum プログラムが asset_data feature を受け付けない（NotAvailable error 6050）ため、代替として URI append 方式を採用している。Bubblegum がこの feature を有効化したら、URI append は廃止して asset_data_hash 経由の binding に切り替える予定。
...
`<license_terms_hash>` は当該 JSON ファイルの keccak256 hash の hex 表記である。第三者は URL を fetch した JSON を keccak256 ハッシュ化し、URL に含まれる hash と一致することを確認することで、条文が改ざんされていないことを検証できる。URL 自体が条文の identity を保管する。
...
```json
{
  ...
  "binding": {
    ...
    "verification_note": "Verify keccak256(b58_decode(this License NFT's bound root_mint)) == leaf.asset_data_hash"
  },
  "terms": {
    ...
    "permitted_uses": ["ai_training", "commercial_use", "..."],
    "duration": "perpetual",
    "governing_law": "Japan",
    "dispute_resolution": "..."
  },
  "full_text": "本ライセンスは、著作権の一方的許諾（unilateral license grant）であり... [全文]"
}
```

ライセンス条文には少なくとも以下を明記する。
...
- ライセンスの対象となるデータの特定方法：「License NFT の `asset_data_hash` から keccak preimage を逆引きして得られる root_asset_id が指す Root NFT に紐づくコンテンツ」と記述する（Layer 1 の binding）。
- 許可される利用範囲（AI モデルの学習利用、商用利用、派生物の生成、再配布など）を定める。種別ごとに異なる URL を用意することで、運用上柔軟に複数のライセンス条項を発行できる。
- ライセンスの有効期間を定める。
- 準拠法と紛争解決条項を定める。
...
4. URI 本体（query 除去）から license JSON を fetch → URL の `<license_terms_hash>` 部分と `keccak256(JSON)` を比較（条文改ざんなし）
5. ステップ 3 で取得した Root NFT を DAS から取得 → TitleCore Collection 所属、TP TEE 経由発行を確認
...
本設計は James Grimmelmann ら（Cornell Tech / IC3 / COALA, 2022）の "Token-Bound NFT License" および a16z の "Can't Be Evil" NFT licenses と同型の構造である。一方的許諾モデル（§5.5.1）+ smart contract による Licensing Process invocation + 暗号学的 hash linking という三要素により、blockchain 上のクリックスルー同意の不在を回避しつつ downstream NFT 保有者にも有効なライセンス継承を実現する。
