...
検出が途切れても前後 3 秒はゾーンを維持する (遮蔽・モーションブラー対策)。 逆に、 同一 id の
目撃がその 3 秒以内にもう 1 回も無い単発の目撃はノイズとして捨てる (= メジャーの黒窓や
キーボードのキー格子などが 4x4 として復号され、 まれにカタログ id に化けるため。 本物の
ステッカーは連続フレームで目撃される)。 マーカーが 1 つも映っていないセッションでは
挙動は従来と同一。
