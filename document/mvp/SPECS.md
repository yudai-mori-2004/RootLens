...
**ビジネスモデル**: C2PA証明付きコンテンツの**P2Pライセンス販売マーケットプレイス**（次世代ストックフォトサイト）
- クリエイターは検証済みコンテンツを直接販売
- 購入者はダウンロードライセンスを購入（何度でも再ダウンロード可能）
- 収益の99.9%がクリエイターへ直接還元（SolanaPay）
...
| **Blockchain** | 所有権の帰属先の明確化、P2P直接取引、譲渡可能性、乗っ取り防止 |

> 重要: C2PAだけでもトラストレスな真正性検証は可能です。ブロックチェーンは「誰のものか」と「P2P直接取引の実現」を担います。
>
...
| **収益権の移転** | Blockchain | cNFTの譲渡により、将来の収益権を第三者に移転可能 |
| **収益化手段の欠如** | Blockchain | Solana Payによる直接取引でクリエイターに99.9%還元 |
...
RootLensの検証ロジックは、単一のレコードに依存するのではなく、**「候補の検索と照合（Search & Match）」** によって行われます。これにより、Mintの失敗やID予測の不整合に対する高い耐障害性を持ちます。

#### なぜ複数のArweaveレコードが存在しうるのか

cNFTのAsset IDを事前に予測してArweaveにアップロードする設計上、以下のような状況で同一`original_hash`に対して複数のArweaveトランザクションが発生します：

- **予測ミス**: 競合やMerkle Treeの状態不整合により、予測したAsset IDと実際にMintされたAsset IDが異なる場合
- **リトライ**: ユーザーが同じファイルを再度アップロードした場合
- **Mint失敗**: ネットワークエラー等でMintが失敗し、再試行された場合

これらの場合、Arweave上には「リンク先のcNFTが存在しない」孤立したレコード（Ghost Record）が残りますが、**検証ロジックはこれを自動的に無視します**。

#### 検証フロー詳細（One-to-Many対応）

```
┌─────────────────────────────────────────────────────────────────┐
│  1. Arweaveから original_hash を持つ全てのトランザクションを取得 │
│     (候補リスト)                                                 │
└─────────────────────────────────────────────────────────────────┘
                                ↓
┌─────────────────────────────────────────────────────────────────┐
│  2. 各候補について、以下を検証:                                   │
│     ├─ 記載された target_asset_id がSolana上に実在するか        │
│     ├─ cNFTが有効（Burnされていない、または正規の履歴がある）か  │
│     └─ cNFTのURIがArweaveトランザクションを指しているか          │
│        (相互リンクの成立)                                        │
└─────────────────────────────────────────────────────────────────┘
                                ↓
┌─────────────────────────────────────────────────────────────────┐
│  3. 「Arweave → Solana」と「Solana → Arweave」の                │
│     双方向リンクが成立するペアを特定                             │
│     → これを正当な証明として採用                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
User: 画像をアップロード
  ↓
Worker: TreeConfig.numMinted = 99 を読み取り
  ↓
Worker: Asset #100 を予測
  ↓
Worker: Arweaveに { target_asset_id: "Asset #100", ... } をアップロード
  ↓
Worker: cNFT Mintを実行
  ↓
【ケース1: 予測成功】
  → 実際のAsset ID: #100
  → Arweave ⇄ Solana の相互リンク成立 ✅
  → 検証成功

【ケース2: 予測失敗】（競合や状態不整合）
  → 実際のAsset ID: #101
  → Arweaveは #100 を指すが、Solana上に #100 は存在しない
  → 相互リンク不成立 ❌
  → この Arweave レコードは「Ghost Record」として無視される

  ユーザーが再アップロード（自動リトライ）:
    → 新しい Arweave TX: { target_asset_id: "Asset #102", ... }
    → Mint成功: Asset #102
    → 相互リンク成立 ✅
    → 検証成功
```
...
検証者
  ↓
Arweave GraphQL検索 (original_hash: "abc123...")
  ↓
┌─────────────────────────────────────────────────────────────────┐
│  検出された候補:                                                  │
│  ├─ TX #1 (Ghost Record)                                        │
│  │   └─ target_asset_id: Asset #100                            │
│  │       ↓                                                      │
│  │       Solana: Asset #100 は存在しない ❌                     │
│  │       → 無視                                                 │
│  │                                                              │
│  └─ TX #2 (Valid Record)                                        │
│      └─ target_asset_id: Asset #101                            │
│          ↓                                                      │
│          Solana: Asset #101 が存在 ✅                           │
│          Solana URI: https://arweave.net/TX#2 ✅                │
│          → 相互リンク成立 → 検証成功                            │
└─────────────────────────────────────────────────────────────────┘
```
...
- **ゴミデータの許容**: 予測が外れたArweaveレコードは「汚点」ではなく、単に「無視されるレコード」として扱われます。Arweaveは永続ストレージであり、削除はできませんが、検証ロジックは正しいペアのみを選び出します。
- **Eventual Consistency（最終的整合性）**: ユーザーがリトライすれば、最終的に整合性の取れたペアが生成され、検証は成功します。
- **コスト効率**: Solana cNFTのMintコストは極小（$0.00005）であり、数回のリトライは許容範囲です。
- **トラストレス性の維持**: 検証ロジックは「Arweaveの記載」と「Solanaの実在性」の両方を確認するため、どちらか一方のデータが壊れていても検証は失敗します。

#### 審査員へのメッセージ

> **「予測が外れたらどうするのか？」という懸念について:**
>
> RootLensは「予測が絶対に当たる」ことを前提としていません。予測が外れても、検証ロジックが複数の候補から正しいペアを探し出すため、システムは破綻しません。これは、SolanaとArweaveの特性（低コスト、永続性）を深く理解した上での実利的な設計です。
