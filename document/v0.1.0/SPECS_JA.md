...
3. **オンチェーンデータ自体は改ざんされていないか？** ── Solanaブロックチェーンの不変性が保証する。オフチェーンデータはTEE署名を内包しており、任意のストレージから取得しても改竄検出が可能
...
公開完了後またはプレビュー画面から、主要SNSに最適化されたシェアフローを提供する。

### 2.5.1 Instagram

1. シェアバーから Instagram アイコンをタップ → Instagram シェア編集画面へ遷移
2. 投稿タイプ（投稿 1:1 / ストーリー 9:16）を選択
3. 固定枠内で画像をピンチ/パンしてクロップ位置を調整
4. ウォーターマーク（QRコード + 「REAL」+ ユーザーネーム）の表示/非表示・サイズを設定
5. 「シェア」ボタンで画像を合成し、OS共有シートで Instagram 等に共有
6. Instagram ではリンクシェアが効かないため、QRコード（検証ページURL）を画像に埋め込む
7. URL はテキスト欄からコピー可能（キャプション用）

### 2.5.2 X（Twitter）— 画像シェア

1. シェアバーから X アイコンをタップ → X シェア編集画面の「画像」タブ
2. アスペクト比（16:9 / 3:4 / 1:1 / 2:1）を選択（X で見切れない比率）
3. クロップ位置を調整、ウォーターマーク（QRコード + 「REAL」+ ユーザーネーム）を設定
4. 「シェア」ボタンで画像を合成し、OS共有シートで共有

### 2.5.3 X（Twitter）— OGPリンクシェア

1. X シェア編集画面の「OGP」タブを選択
2. 1.91:1（1200×630）固定枠でOGP画像のクロップを調整
3. ウォーターマーク（チェックマーク + 「REAL」、QR不要 — リンクから検証ページに到達可能）
4. 「ポスト」ボタンで `x.com/intent/tweet` にリンク + CTAを送信
5. OGPにより画像付きカードがタイムラインに表示される
...
| オフチェーンストレージへのアップロード失敗 | 「公開できませんでした。もう一度お試しください」 | ストレージエラー、アップロード失敗 |
...
- WebViewでrootlens.io上の公開ページを表示
- シェアバー: SNSアイコン直接導線 + ユーティリティ
    - 「Share to」テキスト + Xアイコン + Instagramアイコン → 各SNS専用シェア編集画面へ遷移
    - OS共有シートアイコン + クリップボードアイコン（URLコピー）
    - 3点メニュー → アクションシート（削除等）

### 3.7.1 SNSシェア編集画面

InstagramとXそれぞれに独立した専用画面を持つ。

**共通UI要素:**
- クロッププレビュー（灰背景 + 白余白 + 黒ボーダー + 四隅L字マーク。画像をピンチ/パンで調整）
- ウォーターマーク表示トグル + S/M/L サイズセレクター
- SNSアイコン付きユーザーネーム入力（AsyncStorageで永続化）
- URLコピーボックス
- 全幅シェアボタン

**Instagram（`InstagramShareScreen`）:**
- タブ: 投稿（1:1, 1080×1080）/ ストーリー（9:16, 1080×1920）
- バッジ: QRコード + 「REAL」+ ユーザーネーム
- シェア: 画像を合成 → OS共有シート（`Sharing.shareAsync`）

**X — 画像タブ（`TwitterShareScreen` 画像タブ）:**
- アスペクト比選択: 16:9 / 3:4 / 1:1 / 2:1（Xで見切れない比率）
- バッジ: QRコード + 「REAL」+ ユーザーネーム
- シェア: 画像を合成 → OS共有シート

**X — OGPタブ（`TwitterShareScreen` OGPタブ）:**
- 固定 1.91:1（1200×630）クロップ
- バッジ: チェックマーク + 「REAL」（QR不要 — リンクから検証ページに到達可能）
- シェア: `x.com/intent/tweet` でリンク + CTA を送信
...
| 6 | アプリ（Title Protocol SDK） | 検証結果を確認し、問題なければ当該JSONデータをオフチェーンストレージに保存する |
| 7 | アプリ（Title Protocol SDK） | ストレージのURIをノードに送り返す |
...
- OGP画像サイズ: 1200×630px（`summary_large_image` 推奨サイズ）
- メタデータ: `og:title` = "Real · Verified"、`og:description` = "This photo is not AI. Tap to see proof."、`twitter:card` = "summary_large_image"

### 7.3.1 ウォーターマークバッジ

SNSシェア時に画像右下に付与するバッジ。ユーザーが表示/非表示を選択可能。S/M/Lの3段階サイズ調整。

**共通仕様:**
- 配置: 画像右下、端から12pxマージン
- 背景: #0f1a3c（ネイビー）、角丸10px
- ボーダー: rgba(255,255,255,0.15) 1.5px（暗い写真上でもエッジが消えない）
- パディング: 上下8px、左8px、右12px
- テキスト「REAL」: 白、font-weight 500、letter-spacing 0.14em

**QRコード付きバッジ（Instagram全タブ + X画像タブ）:**
- 左: QRコード（40×40px、白背景角丸5px、#0f1a3cドット、検証URL）
- 右上: 「REAL」
- 右下: ユーザーネーム（rgba(255,255,255,0.5)、小さめ）

**チェックマークバッジ（X OGPタブ）:**
- 左: 白丸（24px）+ ネイビーチェックマーク（QR不要 — OGPリンクで検証ページに到達可能）
- 右上: 「REAL」
- 右下: ユーザーネーム

**描画方式:** `@shopify/react-native-skia` の宣言的 `<Canvas>` コンポーネント + `makeImageSnapshot` によるエクスポート。命令的 API（`Surface.MakeOffscreen`）はAndroidでJSIクラッシュが発生するため不使用。

## 7.4 クライアントサイド検証アーキテクチャ

### 設計原則

公開ページにおける本物証明の表示は、RootLensサーバーから取得したデータに基づいてはならない。公開ページのJavaScriptは、Solana RPCに直接問い合わせてオンチェーンデータを取得し、cNFTのURIからオフチェーンデータを取得し、TEE署名の検証およびpHash照合までをブラウザ内で完結させる。
...
一般ユーザーにとっては、RootLensブランドへの信頼が公開ページを信じる実質的な根拠となる。しかし、疑う人は誰でもブラウザの開発者ツールを開くだけで、公開ページがRootLensサーバーではなくSolana RPCとオフチェーンストレージから直接データを取得していること、および表示画像のpHashがオンチェーンの値と一致していることを確認できる。
...
**公開ページJS自体の完全性について：** 公開ページのHTML/JS/CSSはRootLensサーバーから配信されるため、RootLensがJSを改ざんすれば偽の検証結果を表示できる余地がある。この信頼の境界に対しては、公開ページにcNFT asset IDをそのまま表示することで対処する。閲覧者はこのIDを使い、Solanaエクスプローラーやオフチェーンストレージに直接アクセスして、RootLensのUIを一切経由せずに検証結果を独立に確認できる。RootLensが偽のIDを表示した場合、そのIDでオンチェーンを参照すれば内容の不一致または不存在が即座に判明するため、改ざんは実質的に検出可能である。さらに、pHashがオンチェーンに記録されているため、閲覧者は表示画像のpHashを自分で計算し、オンチェーンの値と突き合わせることで、RootLensが画像とオンチェーンレコードの対応を偽っていないことも独立に確認できる。
...
- cNFTのURIが指すオフチェーンストレージへのデータ取得リクエスト
...
Step 2: Fetching off-chain data...
  → URI: ar://abc123...
...
| オフチェーンストレージ | cNFT紐づきメタデータの保存 | C2PA検証結果、pHash等のExtension結果を含む。TEE署名を内包するため、任意のストレージで改竄検出が可能 |
...
| 出口（記録） | 検証結果の永続化 | Solanaブロックチェーン + オフチェーンストレージ |
