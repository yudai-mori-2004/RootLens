...
function GitHubIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 16 16" fill="currentColor" style={{ width: 18, height: 18, flexShrink: 0 }}>
  ...
  return (
    <article className={s.article}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, margin: "20px 0" }}>
        <a href={GITHUB_TP} target="_blank" rel="noopener noreferrer" className={s.link} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <GitHubIcon />
          {tOs("tpRepo")}
        </a>
        <a href={GITHUB_RL} target="_blank" rel="noopener noreferrer" className={s.link} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <GitHubIcon />
          {tOs("rlRepo")}
        </a>
      </div>
