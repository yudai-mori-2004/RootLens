...
// dataflow の進捗を Metro ログにもミラーする sink (= 撮影画面の Pipeline 進捗)。
...
// キャリブレーション音声ガイドの文言。
//
// 意図 (= 過去の「首で調整」 誤解の是正): これは「ヘッドセット(装着角度)のキャリブレーション」。
// ユーザには『自然な姿勢で自分の手のひらをご覧いただく』 だけ。 その自然な視線状態で手が画面中央に
// 来るようヘッドセットの向きを物理的に合わせるのが目的 (首はひねらせない)。
// トーン = 機内アナウンス調 (= 丁寧・穏やか・誰にでも明確、 砕けすぎない)。 言い回しは Gemini に相談。
...
const CalibrationCaptureBody: React.FC<Props> = ({ navigation }) => {
  ...
  // 画面の向き (= 撮影画面だけ横、 タブは縦) は RootNavigator の native-stack `orientation` オプションが
  // ネイティブ (react-native-screens) で管理する。 expo-screen-orientation の lockAsync は
  // react-native-screens に上書きされて効かないため、 画面側ではロックしない (= 下の native カメラ
  // frame 向きの固定だけ行う)。
    ...
    if (state.kind === 'finalizing') {
      (async () => {
        try {
          ...
          // クリップを起こして dataflow の段レジュームランナーで前進させる (= 背景実行、 撮影ループを止めない)。
          // advanceClip が 未署名 → 撮影署名 → ぼかし署名 (signature_hash 誕生) → 登録 → Pipeline 2 を駆動。
          const capturedConfig = config;
          // 録画尺 = native stop の wall-clock − 録画開始の wall-clock (= 端末申告。 サーバ側 cv2 算出が正だが即時表示用)。
  ...
  // 音と数字を同じ step 判定で同時に出すので、 両者がずれない (= 旧: 音 machine / 数字 別 effect で desync)。
  useEffect(() => {
...
//
// 仕様:
//   - 確定ジェスチャー (= 両手の合議 + 平滑化) のときだけ両手アイコンを表示、 それ以外は非表示
...
// 一緒に出し、 それ以外は非表示)。 片手だけ出る独立表示は「両手出してるのに片手」 ストレスの元なので採らない。
// 平滑化済みなので一瞬の片手落ちでは消えない。
