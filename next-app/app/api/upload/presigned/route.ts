...
interface PresignedUrlRequest {
  ...
  file_type: string; // 'original' | 'c2pa'
  content_type: string; // 'image/jpeg' | 'application/octet-stream' 等
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. R2パス決定
    const extension = file_type === 'c2pa' ? 'c2pa' : getExtensionFromContentType(content_type);
    const key = `media/${original_hash}/${file_type === 'c2pa' ? 'metadata' : 'original'}.${extension}`;
