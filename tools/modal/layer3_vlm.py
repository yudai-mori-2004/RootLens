...
SYSTEM_PROMPT = (
    ...
    "されています。 各フレームに対して以下 4 基準を 0-5 (= 整数) で採点 + 粗いカテゴリを\n"
    "1 つ選択 + 具体的な行動記述文 (description) を生成し、 JSON で返してください。\n"
    "\n"
    "★ description (= 行動記述文) がこのラベルの主役です (= Ego4D 風の dense narration)。\n"
    "  カテゴリは marketplace フィルタ用の粗い目安にすぎません。 意味の中身は description が担います。\n"
    "  description の要件:\n"
    "  - 「どの手で・何を・どう操作しているか」を具体的に書く (= verb + noun を明示)。\n"
    "  - 例: 「右手でスポンジを持ち、 流し台で皿の表面をこすっている」\n"
    "        「両手で洗濯物を畳み、 衣類を重ねて積んでいる」\n"
    "  - 毎フレーム同じ言い回しを避け、 動作の変化を捉えた多様で具体的な表現を使う。\n"
    "  - 字数制限はないが冗長にせず、 1〜2 文で動作を的確に。 何もしていなければ素直にそう書く。\n"
    "\n"
    "4 基準:\n"
    ...
    '      "scene_match": 5,\n'
    '      "category": "cleaning",\n'
    '      "description": "右手でモップの柄を握り、 台所の床を前後に拭いている"\n'
    "    },\n"
    "    ...\n"
    "  ]\n"
    "}\n"
    ...
    "category は上記 8 値のいずれか (= 未知値は禁止、 最も近いものか不明なら other)。 これは粗い目安です。\n"
    "description は固定語彙に縛られず、 具体的で多様な自然文で書いてください (= 主ラベル)。"
)
