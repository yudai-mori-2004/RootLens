...
- 出力: `rootlens-fpvlabs` の `<content_hash>/session.mcap` (EgoBlur 顔ぼかし + ぼかしマーカー (ArUco) の実寸ゾーンぼかし適用済み、ROS2 メッセージの MCAP) + `manifest.jsonl` 再生成 (バケット直下の属性表。DB + R2 から毎回全再生成)
