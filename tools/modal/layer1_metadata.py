...
入力:  R2 raw bucket の `raw/<signature_hash>/realtime_handpose.jsonl` (= 2026-05-28: IMU 撤去で frames のみ)
処理:  JSONL を 1 行ずつ stream parse して 4 指標を集計、 各 0..1 正規化 × 配点 を合算
出力:  Layer1Score JSON (= camelCase キー、 score は 0..20 の整数)

4 指標 (= document/v0.1.3/DATA_SPECS_JA §3.2.4 第 1 層内訳、 2026-05-28 IMU + ARKit 撤去後):

| 指標                          | 配点 | 算出                                                                      |
|-------------------------------|------|---------------------------------------------------------------------------|
| handLandmarkPresenceBoth      | 9    | 両手 21x2 = 42 ランドマーク全てフレーム内のフレーム割合                   |
| rgbSensorSyncRatio            | 4    | 有効な timestamp_ns + frame_index 行の割合                                |
| frameContinuity               | 4    | 1 - 欠番率 (= 0..max(frame_index) の期待列に対する穴の割合)               |
| handMovement                  | 3    | 手首ランドマーク変位の分散を閾値 (= 100 px²) で正規化 (clip 0..1)          |

2026-05-27 撤去: trackingQuality (= ARKit tracking_state) ─ ARKit 廃止
2026-05-28 撤去: imuGravityCompliance (= IMU) ─ 時刻同期破綻 + Pipeline 3 未活用、 配点 1 点は handLandmarkPresenceBoth に振替

設計判断:

- **棄却閾値なし**。 低スコアでも score を返す。
- **空ファイル / 不完全データは 0 で返す** (= 例外を吐かない)。 全フィールドが 0 の Layer1Score が「frames が読めなかった / 中身がほぼ空」 のシグナル。
...
WEIGHT_HAND_PRESENCE = 9
WEIGHT_SYNC_RATIO = 4
WEIGHT_FRAME_CONTINUITY = 4
WEIGHT_HAND_MOVEMENT = 3
WEIGHT_TOTAL = (
...
assert WEIGHT_TOTAL == 20, "配点の合計は 20 点"
...
def _compute_sensors_metrics(sensors_text: str) -> dict[str, float]:
    ...
    for line in sensors_text.splitlines():
        ...
        # 2026-05-27 撤去: tracking_state (= ARKit) は realtime_handpose.jsonl に存在しない
...
# 2026-05-28 撤去: _compute_imu_metrics (= imuGravityCompliance) ─ IMU は時刻同期破綻で削除
...
def _combine_score(metrics: dict[str, float]) -> int:
    """4 指標 (= 0..1) と配点から score (= 0..20 整数) を計算。"""
    total = (
...
def score(signature_hash: str):
    """
    Pipeline 2 第 1 層 entry point。

    Args (query string):
        signature_hash: クリップの signature_hash (= R2 prefix raw/<signature_hash>/ を読む)
    ...
    # 入力 validation: signature_hash が無い / path traversal を弾く。
    # 例外を吐かないポリシーなので、 不正でも zero score を返す。
    if not isinstance(signature_hash, str) or not signature_hash or "/" in signature_hash or ".." in signature_hash:
        return _zero_score()
    ...
    # 2026-05-28: 4 → 2 ファイル集約で realtime_handpose.jsonl のみ。 imu_high_rate.jsonl の fetch は撤去。
    frames_key = f"raw/{signature_hash}/realtime_handpose.jsonl"
    frames_text = _fetch_text(s3, bucket_raw, frames_key)

    try:
        metrics = _compute_sensors_metrics(frames_text)
    except Exception:
        metrics = {
            "handLandmarkPresenceBoth": 0.0,
            "rgbSensorSyncRatio": 0.0,
