# pipeline-showcase: raw セッション → LP のショーケースページ用のマルチチャンネル素材。
#
# rootlens-raw-arkit/raw/<content_hash>/ を読み、 /sample の 4 パネル同期ビューアが必要とする
# 6 つのアセットを組み立てて rootlens-public/lp-sample/<slug>/ に配置する。
...
#
# 実行:
#   Modal:  modal run tools/modal/showcase/showcase.py \
#             --content-hash <hash> --slug arkit-home-01
#           modal run tools/modal/showcase/showcase.py \
#             --content-hash <hash> --slug test-01 --target-bucket rootlens-public-staging
#   ローカル: FPVLABS_LOCAL=1 python tools/modal/showcase/showcase.py <hash> <slug>
#
...
def build_mesh(src_jsonl: str, out_glb: str) -> dict:
    ...
    gltf = {
        "asset": {"version": "2.0", "generator": f"rootlens-showcase/{PIPELINE_VERSION}"},
        "scene": 0,
        "scenes": [{"nodes": [0]}],
        "nodes": [{"mesh": 0}],
        "meshes": [{
...
try:
    ...
    app = modal.App("rootlens-showcase")
