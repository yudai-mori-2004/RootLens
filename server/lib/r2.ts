...
import { rawMp4Key, blurredMp4Key, deliveryMcapKey } from "./r2-keys";
...
// サーバは事前署名 URL の発行と、 サーバ自身が R2 を読み書きする必要がある時 (= blur / synthesize の
// Modal 経由 + TP submit 時の R2 download) だけ S3Client を使う。
//
// バケット構成 (= SPECS §6.2 ステップ 7):
//   R2_BUCKET_RAW       : 端末 upload された 生 MP4 (= 非公開、 短期 retention)
//   R2_BUCKET_BLURRED   : サーバ処理後の ぼかし済 MP4 + 合成 MCAP (= buyer 配信)
//                          - blurred-mp4/<contentHash>.mp4
//                          - delivery-mcap/<contentHash>.mcap
...
// key 命名関数は lib/r2-keys.ts に分離。 互換性のためここからも re-export する。
export { rawMp4Key, blurredMp4Key, deliveryMcapKey };
...
/// 任意のキーに対する blurred バケット GET 事前署名 URL (= preview MP4 / 配信 MCAP 等)。
...
/// 配信 MCAP の GET URL (= buyer が License NFT 保有確認後に取得)。
export async function presignDeliveryMcapGet(opts: {
  contentHash: string;
  expiresInSec?: number;
}): Promise<{ url: string; key: string }> {
  const key = deliveryMcapKey(opts.contentHash);
  const cmd = new GetObjectCommand({ Bucket: BUCKET_BLURRED, Key: key });
  const url = await getSignedUrl(r2, cmd, { expiresIn: opts.expiresInSec ?? 3600 });
  return { url, key };
}

export { r2, BUCKET_RAW, BUCKET_BLURRED };
