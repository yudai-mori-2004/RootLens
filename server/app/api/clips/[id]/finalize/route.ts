...
// 端末が R2 への PUT を完了したら呼ぶ。 サーバはハッシュ照合 + state 遷移 + workflow 起動。
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  if (clip.contentHash !== parsed.data.contentHash) {
    return NextResponse.json(
      { error: "Content hash mismatch (expected " + clip.contentHash + ")" },
      { status: 409 },
    );
  }
